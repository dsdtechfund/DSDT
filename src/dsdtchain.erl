...
status([Node]) ->
    ...
    io:format("blockchain is of height ~p with head ~s~n", [CurrentHead#block.height, dsdtchain_sync_handler:hexdump(hash_block(CurrentHead))]),
...
handle_info({mined_block, NewBlock, Addr}, State) ->
    ...
    case validate_chain(NewBlock, State#state.blockchain) of
        {error, {missing_block, _Hash}} when Addr /= self ->
            Path = "dsdtchain_sync/1.0.0/" ++ erlang:integer_to_list(CurrentHead#block.height) ++ "/" ++ dsdtchain_sync_handler:hexdump(hash_block(CurrentHead)),
            case libp2p_swarm:dial(State#state.swarm, Addr, Path) of
                {ok, Conn} ->
                    libp2p_framed_stream:client(dsdtchain_sync_handler, Conn, [self()]);
                Other ->
        ...
        {NewLedger, ProposedHead} when CurrentHead#block.height < ProposedHead#block.height ->
            ...
            lager:info("Head is now ~w", [dsdtchain_sync_handler:hexdump(hash_block(ProposedHead))]),
...
start_swarm_server(Name, [], {PrivKey, PubKey}) ->
    ...
    ok = libp2p_swarm:add_stream_handler(Swarm, "dsdtchain/1.0.0", {libp2p_framed_stream, server, [dsdtchain_handler, self()]}),
    ok = libp2p_swarm:add_stream_handler(Swarm, "dsdtchain_sync/1.0.0", {libp2p_framed_stream, server, [dsdtchain_sync_handler, self()]}),
...
start_swarm_server(Name, SeedNodes, {PrivKey, PubKey}) ->
    ...
    Opts = [
        ...
        ,{libp2p_group_gossip, [
            {stream_clients, [
                {"dsdtchain/1.0.0", {dsdtchain_handler, [self()]}}
            ]}
